...
class CarcassonneMap:
    ...
    def trace_road_one_direction(self, x, y, side):
        """ Follows a road path within the map of tiles. Encountering a
        crossroad or loop marks the end point.

        Parameters:
            x: the x coordinate of the path's starting tile
            y: the y coordinate of the path's starting tile
            side: the exiting side of the path's starting tile
        """
        traveled = []
        start = (x, y)
        leaving = side
                ...
                for coord, route in check.items():
                    if coord in self._map.keys() and route[0] == leaving:
                        if coord not in no_repeats:
                            ending = self._map.get(coord).\
                                road_get_connection(route[1])
    ...
    def trace_road(self, x, y, side):
        """ Follows a road path in multiple directions, finding both ends to
        the path. Encountering a crossroad or loop marks the end point.
        ...
        Parameters:
            ...
            side: the exiting side of the path's starting tile
        """
        opposite = self.get(x, y).road_get_connection(side)
        origin = (x, y, opposite, side)

        forward_path = (self.trace_road_one_direction(x, y, side))
        reverse_path = (self.trace_road_one_direction(x, y, opposite))

        swapped = []
        for tup in reverse_path:
            start_end = list(tup)
            start_end[2], start_end[3] = start_end[3], start_end[2]
            swapped.append(tuple(start_end))

        # Checks for loops
        fp_set = set(forward_path)
        rp_set = set(swapped)
        loop = fp_set.intersection(rp_set)
        if loop:
            return forward_path
        else:
            swapped.reverse()
            swapped.append(origin)
            ret_path = swapped + forward_path
            return ret_path